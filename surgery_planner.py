...
def plan(requests: list[dict], profile: pd.DataFrame, start: date, horizon_days: int = 14,
         already: dict[tuple[str, str], int] | None = None) -> list[dict]:
    """Propuesta: [{id, fecha, area, prioridad, espera_dias, alerta}]. `already` = cupos ya usados por
    cirugías programadas en la app {(area, fecha): n}."""
    ...
    for r in order:
        ...
        for d in window:
            ...
            if used.get(key, 0) < free.get((r["area_quirofano"], d.weekday()), 0):
                chosen = d
                break
        ...
        out.append({"id": r["id"], "paciente": r.get("paciente"), "area": r["area_quirofano"],
                    "prioridad": r["prioridad"], "fecha": chosen.isoformat() if chosen else None,
                    "espera_dias": max(waited, 0), "alerta": alert})
    return sorted(out, key=lambda x: (x["fecha"] or "9999", x["area"]))
    ...
    with conn:
        for p in proposal:
            ...
            cur = conn.execute("UPDATE cirugias_solicitudes SET estado = 'PROGRAMADA', fecha_programada = ?, "
                               "actualizado_por = ?, actualizado_en = ? WHERE id = ? AND estado = 'EN_ESPERA'",
                               (p["fecha"], user_id, now, p["id"]))
            if cur.rowcount:
                n += 1
                pid = conn.execute("SELECT id_paciente FROM cirugias_solicitudes WHERE id = ?", (p["id"],)).fetchone()[0]
                _event(conn, pid, f"Cirugía programada para el {p['fecha']} ({AREAS.get(p['area'], p['area'])})",
                       user_id, now)
    return n
...
def schedule(conn: sqlite3.Connection, request_id: int, day: str, user_id: int, now: str, profile: pd.DataFrame,
             *, coordinator: bool, justification: str = "") -> None:
    """Agendar una cirugía en un día. Reglas:
    - solo coordinación de quirófanos agenda; la solicitud debe estar en espera;
    - no en el pasado ni a más de 90 días; el paciente no puede tener otra cirugía programada ese día;
    ...
    with conn:
        conn.execute("UPDATE cirugias_solicitudes SET estado = 'PROGRAMADA', fecha_programada = ?, "
                     "sobrecupo_justificacion = ?, actualizado_por = ?, actualizado_en = ? WHERE id = ?",
                     (day, just if reasons else None, user_id, now, request_id))
        _event(conn, r["id_paciente"], f"Cirugía programada para el {day} ({AREAS.get(r['area_quirofano'])})"
...
def reprogram(conn: sqlite3.Connection, request_id: int, user_id: int, now: str, motivo: str, *,
    ...
    with conn:
        conn.execute("UPDATE cirugias_solicitudes SET estado = 'EN_ESPERA', fecha_programada = NULL, "
                     "sobrecupo_justificacion = NULL, motivo = ?, actualizado_por = ?, actualizado_en = ? WHERE id = ?",
