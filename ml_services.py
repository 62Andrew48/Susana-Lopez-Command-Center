...
def describe(service: Service, pred: dict, kpi_data: dict | None = None) -> str:
    """Respuesta en lenguaje claro a partir del contrato /predict (y /kpis si está disponible)."""
    import forecast_text as ft

    r = ft.interpret(service.code, pred, kpi_data)
    text = (f"**{service.name}.** {r.headline} {r.range_text}\n\n"
            f"**Frente a lo habitual: {r.level.lower()}.** {r.level_text}\n\n"
            f"**{r.confidence}.** {r.confidence_text}")
    if r.actions:
